...
#line 26 "..\\common\\completion.c"
#line 27 "..\\common\\completion.c"
...
{
    rt_base_t level;
    if (!(completion != (0))) { rt_assert_handler("completion != RT_NULL", __FUNCTION__, 35); };
                            ...
                            rt_int32_t            timeout)
{
    ...
    rt_base_t level;
    rt_thread_t thread;
    if (!(completion != (0))) { rt_assert_handler("completion != RT_NULL", __FUNCTION__, 50); };
    ...
    if (completion->flag != 1)
    {
        ...
        if (!(rt_list_isempty(&(completion->suspended_list)))) { rt_assert_handler("rt_list_isempty(&(completion->suspended_list))", __FUNCTION__, 59); };
...
{
    rt_base_t level;
    if (!(completion != (0))) { rt_assert_handler("completion != RT_NULL", __FUNCTION__, 114); };
