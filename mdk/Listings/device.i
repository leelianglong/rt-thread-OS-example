#line 1 "..\\rt_thread\\kernel\\device.c"
...
#line 32 "..\\rt_thread\\kernel\\device.c"
...
rt_err_t rt_device_unregister(rt_device_t dev)
{
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 72); };
...
rt_err_t rt_device_init(rt_device_t dev)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 149); };
...
rt_err_t rt_device_open(rt_device_t dev, rt_uint16_t oflag)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 184); };
    ...
    {
        dev->open_flag = oflag | 0x008;

        dev->ref_count++;
        
 
        if (!(dev->ref_count != 0)) { rt_assert_handler("dev->ref_count != 0", __FUNCTION__, 225); };
    }
...
rt_err_t rt_device_close(rt_device_t dev)
{
    ...
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 243); };
...
rt_size_t rt_device_read(rt_device_t dev,
                         ...
                         rt_size_t   size)
{
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 284); };
...
rt_size_t rt_device_write(rt_device_t dev,
                          ...
                          rt_size_t   size)
{
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 322); };
...
rt_err_t rt_device_control(rt_device_t dev, rt_uint8_t cmd, void *arg)
{
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 354); };
...
rt_device_set_rx_indicate(rt_device_t dev,
                          rt_err_t (*rx_ind)(rt_device_t dev, rt_size_t size))
{
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 379); };
...
rt_device_set_tx_complete(rt_device_t dev,
                          rt_err_t (*tx_done)(rt_device_t dev, void *buffer))
{
    if (!(dev != (0))) { rt_assert_handler("dev != RT_NULL", __FUNCTION__, 400); };
