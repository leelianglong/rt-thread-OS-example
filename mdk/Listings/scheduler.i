#line 1 "..\\rt_thread\\kernel\\scheduler.c"
...
#line 41 "..\\rt_thread\\kernel\\scheduler.c"
...
#line 42 "..\\rt_thread\\kernel\\scheduler.c"
...
{
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 90); };
...
#line 106 "..\\rt_thread\\kernel\\scheduler.c"
...
void rt_system_scheduler_init(void)
{
    ...
    do { if (0) rt_kprintf ("start scheduler: max priority 0x%02x\n", 32); } while (0);
...
#line 166 "..\\rt_thread\\kernel\\scheduler.c"
...
void rt_schedule(void)
{
    ...
    if (rt_scheduler_lock_nest == 0)
    {
        ...
        highest_ready_priority = __rt_ffs(rt_thread_ready_priority_group) - 1;
#line 214 "..\\rt_thread\\kernel\\scheduler.c"
        ...
        if (to_thread != rt_current_thread)
        {
            ...
            do { if (0) rt_kprintf ("[%d]switch to priority#%d " "thread:%.*s(sp:0x%p), " "from thread:%.*s(sp: 0x%p)\n", rt_interrupt_nest, highest_ready_priority, 8, to_thread->name, to_thread->sp, 8, from_thread->name, from_thread->sp); } while (0);
#line 237 "..\\rt_thread\\kernel\\scheduler.c"
            ...
            {
                do { if (0) rt_kprintf ("switch in interrupt\n"); } while (0);
...
void rt_schedule_insert_thread(struct rt_thread *thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 272); };
    ...
    do { if (0) rt_kprintf ("insert thread[%.*s], the priority: %d\n", 8, thread->name, thread->current_priority); } while (0);
#line 297 "..\\rt_thread\\kernel\\scheduler.c"
...
void rt_schedule_remove_thread(struct rt_thread *thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 318); };

     
    temp = rt_hw_interrupt_disable();


    do { if (0) rt_kprintf ("remove thread[%.*s], the priority: %d\n", 8, thread->name, thread->current_priority); } while (0);
#line 336 "..\\rt_thread\\kernel\\scheduler.c"

     
    rt_list_remove(&(thread->tlist));
    if (rt_list_isempty(&(rt_thread_priority_table[thread->current_priority])))
    {
#line 348 "..\\rt_thread\\kernel\\scheduler.c"
        rt_thread_ready_priority_group &= ~thread->number_mask;
