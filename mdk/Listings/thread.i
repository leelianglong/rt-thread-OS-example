#line 1 "..\\rt_thread\\kernel\\thread.c"
...
#line 41 "..\\rt_thread\\kernel\\thread.c"
...
#line 42 "..\\rt_thread\\kernel\\thread.c"
                                ...
                                rt_uint32_t       tick)
{
    ...
    if (!(priority < 32)) { rt_assert_handler("priority < RT_THREAD_PRIORITY_MAX", __FUNCTION__, 110); };
                        ...
                        rt_uint32_t       tick)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 168); };
    if (!(stack_start != (0))) { rt_assert_handler("stack_start != RT_NULL", __FUNCTION__, 169); };
...
rt_err_t rt_thread_startup(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 206); };
    if (!(thread->stat == 0x00)) { rt_assert_handler("thread->stat == RT_THREAD_INIT", __FUNCTION__, 207); };
    ...
    do { if (0) rt_kprintf ("startup a thread:%s with priority:%d\n", thread->name, thread->init_priority); } while (0);
...
rt_err_t rt_thread_detach(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 250); };
...
rt_err_t rt_thread_delete(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 345); };
...
rt_err_t rt_thread_sleep(rt_tick_t tick)
{
    ...
    thread = rt_current_thread;
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 430); };
...
rt_err_t rt_thread_control(rt_thread_t thread, rt_uint8_t cmd, void *arg)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 481); };
...
rt_err_t rt_thread_suspend(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 559); };

    do { if (0) rt_kprintf ("thread suspend:  %s\n", thread->name); } while (0);

    if (thread->stat != 0x01)
    {
        do { if (0) rt_kprintf ("thread suspend: thread disorder, %d\n", thread->stat); } while (0);
...
rt_err_t rt_thread_resume(rt_thread_t thread)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 600); };

    do { if (0) rt_kprintf ("thread resume:  %s\n", thread->name); } while (0);

    if (thread->stat != 0x02)
    {
        do { if (0) rt_kprintf ("thread resume: thread disorder, %d\n", thread->stat); } while (0);
...
void rt_thread_timeout(void *parameter)
{
    ...
    if (!(thread != (0))) { rt_assert_handler("thread != RT_NULL", __FUNCTION__, 643); };
    if (!(thread->stat == 0x02)) { rt_assert_handler("thread->stat == RT_THREAD_SUSPEND", __FUNCTION__, 644); };
